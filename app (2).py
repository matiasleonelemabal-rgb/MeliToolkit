...
def mostrar_tablero_rentabilidad_ag():
    st.subheader("Rentabilidad por venta")
    st.caption(
        "Subí el detalle de ventas de tu sistema (una línea por producto vendido, "
        "con precio, costo y % de profit)."
    )

    archivo = st.file_uploader(
        "Archivo de detalle de ventas (xlsx)",
        type=["xlsx"],
        key="ag_rentabilidad_uploader"
    )
...
def mostrar_tablero_evolucion_ag():
    st.subheader("Evolución del negocio")
    st.caption(
        "Subí el 'Reporte de evolución del negocio' descargado desde Mercado Libre "
        "(hoja 'Negocio': visitas, compradores, recompra por día)."
    )

    archivo = st.file_uploader(
        "Archivo de evolución de negocio (xlsx)",
        type=["xlsx"],
        key="ag_evolucion_uploader"
    )
...
def mostrar_tablero_top_productos_ag():
    st.subheader("Top productos")
    st.caption(
        "Subí el reporte 'Ventas por Producto' descargado desde Mercado Libre."
    )

    archivo = st.file_uploader(
        "Archivo de ventas por producto (xlsx)",
        type=["xlsx"],
        key="ag_top_uploader"
    )
...
def mostrar_herramienta_analista_general():
    ...
    st.caption(
        "Fase 1: tres tableros independientes. Todavía no cruza margen interno "
        "por pieza contra performance de Meli por kit/publicación (usan códigos "
        "de producto distintos) — eso es la fase 2, pendiente de definir."
    )

    tablero = st.radio(
        "¿Qué querés analizar?",
        options=["rentabilidad", "evolucion", "top"],
        format_func=lambda v: {
            "rentabilidad": "Rentabilidad por venta (tu sistema)",
            "evolucion": "Evolución del negocio (Meli)",
            "top": "Top productos (Meli)",
        }[v],
        key="ag_tablero",
        horizontal=True,
    )
    ...
    if tablero == "rentabilidad":
        mostrar_tablero_rentabilidad_ag()
    elif tablero == "evolucion":
        mostrar_tablero_evolucion_ag()
    else:
        mostrar_tablero_top_productos_ag()
