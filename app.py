...
# Carpetas donde puede estar el AGENTE committeado en el repo
_AGENTE_DIRS = [
    _APP_DIR,
...
def _ruta_agente_repo():
    """
    Busca el AGENTE committeado en el repo. Acepta cualquier variante del
    nombre (con espacio o guión bajo, mayúsculas/minúsculas): matchea todo
    .xlsx cuyo nombre contenga 'agente' y 'publicador'. Ignora la copia
    'guardado' escrita en runtime.
    """
    for d in _AGENTE_DIRS:
        ...
        for fn in os.listdir(d):
            if not fn.lower().endswith(".xlsx"):
                continue
            n = normalizar_texto(fn)
            if "guardado" in n:
                continue
            if "agente" in n and "publicador" in n:
                candidatos.append(fn)
        if candidatos:
            return os.path.join(d, sorted(candidatos)[0])
    return None
...
# ============================================================
# GENERADOR DE DESCRIPCIONES DE KITS
# ============================================================
# Data de componentes y templates adaptados por categoría

COMPONENTES_KITS = {
    "crique_carrito_2t": {
        "nombre": "Crique Hidráulico Tipo Carrito 2 Toneladas",
        "descripcion_corta": "crique hidráulico tipo carrito 2 toneladas",
        "descripcion_tecnica": """CRIQUE HIDRÁULICO TIPO CARRITO 2 TONELADAS

El crique hidráulico tipo carrito 2 Toneladas permite elevar el vehículo de forma estable y controlada durante tareas de auxilio vial, mantenimiento básico o cambio de rueda.

- Capacidad de carga de 2 Toneladas, equivalente a 2000 kg.

- Mecanismo hidráulico para realizar la maniobra de elevación.

- Válvula de seguridad incorporada.

- Ruedas traseras giratorias que facilitan el posicionamiento del crique debajo del vehículo.

- Formato tipo carrito que permite desplazar y acomodar la herramienta durante la maniobra de elevación.

MEDIDAS APROXIMADAS

- Largo del crique: 42 cm aprox.

- Ancho delantero: 13,5 cm aprox., medido de extremo a extremo del sector delantero.

- Ancho trasero: 19 cm aprox., medido de extremo a extremo del sector de ruedas del lado de la palanca.

- Altura mínima: 13 cm aprox.

- Altura máxima: 32 cm aprox.

- Capacidad de carga: 2 Toneladas, equivalente a 2000 kg.

- Largo de la palanca: 38 cm aprox.

- Diámetro de la palanca: 1,8 cm aprox.

- Diámetro del agarre de la palanca: 2,3 cm aprox.""",
    },
    "llave_cruz": {
        "nombre": "Llave Cruz Niquelada 4 Medidas 17, 19, 21 y 23 mm",
        "descripcion_corta": "llave cruz niquelada con bocas de 17, 19, 21 y 23 mm",
        "descripcion_tecnica": """LLAVE CRUZ NIQUELADA 4 MEDIDAS 17, 19, 21 Y 23 MM

La llave cruz niquelada permite aflojar y ajustar tuercas hexagonales de rueda durante tareas de cambio de neumáticos, auxilio vial y mantenimiento automotriz.

- Incluye 1 llave cruz niquelada.

- Diseño en cruz que permite aplicar fuerza durante la maniobra de ajuste o desmontaje de la rueda.

- Fabricada en acero forjado de alta resistencia.

- Acabado niquelado opaco.

- Formato 4 en 1 con cuatro bocas para tuercas hexagonales en una sola herramienta.

- Ideal para llevar en el baúl o compartimiento de herramientas del vehículo.

MEDIDAS APROXIMADAS

- Largo: 35,5 cm aprox.

- Boca para tuercas hexagonales: 23 mm.

- Boca para tuercas hexagonales: 21 mm.

- Boca para tuercas hexagonales: 19 mm.

- Boca para tuercas hexagonales: 17 mm""",
    },
    "guantes_trabajo": {
        "nombre": "Guantes de Trabajo Antideslizantes con Puntos de PVC",
        "descripcion_corta": "guantes de trabajo antideslizantes con puntos de pvc en la palma",
        "descripcion_tecnica": """GUANTES DE TRABAJO ANTIDESLIZANTES

Los guantes de trabajo brindan protección y agarre durante tareas de mecánica ligera, manipulación de herramientas, carga, descarga y trabajos generales.

- Incluye 1 par, compuesto por 2 guantes.

- Diseño para 5 dedos: pulgar, índice, medio, anular y meñique.

- Puntos de PVC antideslizantes ubicados en la palma de la mano para mejorar el agarre de herramientas y piezas.

- El dorso de la mano no posee puntos de PVC antideslizantes.

- Tejido de algodón y poliéster.

- Puños elásticos que favorecen el ajuste del guante.

- Talle único de uso general.
...
- Ancho: 13 cm aprox.""",
    ...
    "chaleco_reflectivo": {
        "nombre": "Chaleco Reflectivo de Alta Visibilidad",
        "descripcion_corta": "chaleco reflectivo de alta visibilidad con bandas reflectantes",
        "descripcion_tecnica": """CHALECO REFLECTIVO DE ALTA VISIBILIDAD

El chaleco reflectivo permite mejorar la visibilidad del usuario durante detenciones, tareas de auxilio vial y situaciones de emergencia en ruta, ciudad o vía pública.

- Incluye 1 chaleco reflectivo.

- Color flúor vibrante de alta visibilidad.

- Bandas reflectantes de 360° que favorecen la visibilidad del usuario.

- Fabricado en poliéster de malla ligera y respirable.

- Diseño amplio para facilitar su colocación sobre otras prendas.

- Talle universal de uso general.
...
- Ancho: 56 cm aprox.""",
    ...
    "baliza_reflectiva": {
        "nombre": "Baliza Reflectiva Homologada con Caja Protectora",
        "descripcion_corta": "baliza reflectiva homologada con patas metálicas extensibles y caja protectora",
        "descripcion_tecnica": """BALIZA REFLECTIVA HOMOLOGADA

La baliza reflectiva permite señalizar la presencia de un vehículo detenido durante situaciones de emergencia, auxilio vial o detenciones en ruta y vía pública.

- Incluye 1 baliza reflectiva homologada.

- Fabricada con material acrílico rojo y tela reflectiva.

- Estructura plegable que facilita el armado y guardado.

- Cuenta con 4 patas o soportes metálicos extensibles.

- Incorpora 2 soportes delanteros y 2 soportes traseros.

- Incluye caja protectora roja para facilitar el almacenamiento y guardado en el vehículo.

MEDIDAS APROXIMADAS

- Ancho de la baliza: 45 cm aprox.

- Alto de la baliza: 39 cm aprox.

- Largo con patas extendidas, medido de extremo a extremo: 74,5 cm aprox.

- Largo de la caja de guardado: 45 cm aprox.

- Alto de la caja de guardado: 4 cm aprox.

- Ancho de la caja de guardado: 3 cm aprox.""",
    },
    "cono_reflectivo_plegable": {
        "nombre": "Cono Reflectivo Plegable de 40 cm",
        "descripcion_corta": "cono reflectivo plegable de 40 cm con banda reflectiva y base plástica",
        "descripcion_tecnica": """CONO REFLECTIVO PLEGABLE DE 40 CM

El cono reflectivo plegable permite señalizar detenciones, emergencias viales y sectores de trabajo, combinando visibilidad con un formato compacto para su guardado.

- Incluye 1 cono reflectivo plegable.

- Tela color naranja flúor de alta visibilidad.

- Banda reflectiva de alta intensidad que favorece la visibilidad durante la señalización.

- Diseño plegable que permite reducir el tamaño del cono para facilitar su transporte y guardado.

- Base cuadrada plástica que proporciona estabilidad y resistencia.

- Ideal para llevar en el baúl del vehículo y utilizar durante situaciones de auxilio vial o emergencia.

MEDIDAS APROXIMADAS

- Alto del cono desplegado: 40 cm aprox.

- Ancho de la base plástica: 24,5 x 24,5 cm aprox.

- Alto de la base plástica: 4 cm aprox.""",
    },
    "minicompresor_12v": {
        "nombre": "Compresor de Aire Portátil 12V con Manómetro",
        "descripcion_corta": "compresor de aire portátil 12v con manómetro y kit de 3 picos adaptadores",
        "descripcion_tecnica": """COMPRESOR DE AIRE PORTÁTIL 12V CON MANÓMETRO

El compresor de aire portátil 12V permite inflar neumáticos y otros elementos inflables mediante su conexión a la ficha de encendedor de 12V del vehículo.

- Incluye 1 compresor de aire portátil 12V.

- Cuerpo fabricado en metal, aleación de aluminio y plástico.

- Manómetro incorporado en la parte superior del cuerpo del compresor para visualizar la presión durante el inflado.

- Botón de encendido y apagado ON/OFF.

- Manija superior que facilita el agarre y transporte del compresor.

- Cable de energía con conexión para ficha de encendedor de 12V.

- Incluye kit de 3 picos adaptadores para inflar neumáticos y diferentes elementos deportivos o recreativos.

DATOS DE ETIQUETA

- Voltaje: DC 12V.

- Amperaje máximo: 10 Amp.
...
- Largo del compresor: 16 cm aprox.

- Alto del compresor: 14 cm aprox.

- Ancho del compresor: 8 cm aprox.

- Largo del cable de energía 12V: 260 cm aprox.

- Largo de la manguera de aire: 90 cm aprox.

- Diámetro del manómetro: 5 cm aprox.""",
    },
}

TEMPLATES_DESCRIPCION_KIT = {
    "kit_seguridad": {
        "titulo_format": "KIT DE SEGURIDAD PARA VEHÍCULOS Y AUXILIO VIAL{con_componentes}",
        "intro": """Kit de seguridad para vehículos y auxilio vial compuesto por {componentes_lista}, ideal para llevar en el baúl ante emergencias, cambio de neumáticos, detenciones imprevistas, control de presión, inflado y tareas básicas de asistencia vehicular.

Este conjunto reúne herramientas y elementos de apoyo para responder ante diferentes situaciones de uso en ruta, ciudad, cochera o viaje, combinando múltiples funciones de seguridad, elevación del vehículo, inflado portátil y elementos de visibilidad o señalización.

Pensado para quienes buscan centralizar en un solo kit distintos elementos de auxilio vial y uso preventivo dentro del vehículo.""",
        "include_list": True,
    },
    "compresor": {
        "titulo_format": "COMPRESOR PORTÁTIL 12V{con_componentes}",
        "intro": """Compresor de aire portátil 12V{con_resto}. El compresor es el elemento principal de esta publicación y permite realizar tareas de inflado utilizando la alimentación 12V del vehículo.

Su formato portátil, el manómetro incorporado y los 3 picos adaptadores permiten utilizarlo en neumáticos y otros elementos inflables compatibles.{complementos}

Ideal para llevar en el baúl como equipo preventivo para viajes, uso cotidiano y asistencia mecánica básica.""",
        "include_list": True,
    },
    "crique": {
        "titulo_format": "CRIQUE HIDRÁULICO TIPO CARRITO 2 TONELADAS{con_componentes}",
        "intro": """Crique hidráulico tipo carrito 2 Toneladas{con_resto}. El crique es la herramienta principal de esta publicación y permite elevar el vehículo de forma estable y controlada durante cambio de rueda, mantenimiento básico y tareas de auxilio vial.{complementos}

Ideal para llevar en el baúl como equipo preventivo para viajes, uso cotidiano y cambio de neumáticos.""",
        "include_list": True,
    },
}

CIERRE_ESTANDAR = """FACTURACIÓN AUTOMATIZADA

Emitimos Factura A y B. El proceso está automatizado y tomará directamente los datos fiscales cargados en tu cuenta al momento de ejecutar la compra.

LOGÍSTICA Y DESPACHO INMEDIATO

Envíos Flex: compras previas a las 13:00 hs se entregan en el día, CABA y GBA, entre las 16:00 hs y 22:00 hs.

Mercado Envíos: despachamos a todo el país en horario comercial con embalaje de alta densidad para proteger los productos durante el tránsito.

GARANTÍA

Nuestros productos cuentan con garantía directa del fabricante por 30 días desde la fecha de compra.

IMPORTANTE

Para un uso seguro, el crique debe utilizarse sobre una superficie firme, plana y estable. Siempre respetar los puntos de apoyo indicados por el fabricante del vehículo.

Nunca permanecer debajo de un vehículo sostenido únicamente por el crique hidráulico.

Antes de realizar el inflado, verificá la presión recomendada por el fabricante del neumático o del elemento a inflar.

Las imágenes son ilustrativas y las medidas expresadas son aproximadas."""


def generar_descripcion_kit(componentes_elegidos, categorias=None):
    """
    Genera descripciones para múltiples categorías o una sola.
    
    componentes_elegidos: lista ["crique_carrito_2t", "minicompresor_12v", "llave_cruz", ...]
    categorias: string ("kit_seguridad") o lista ["kit_seguridad", "compresor"] o None (todas)
    
    Devuelve: dict {cat: descripcion} si input es lista, string si input es string
    """
    if not componentes_elegidos:
        return {} if isinstance(categorias, list) else ""
    
    comps_validos = [c for c in componentes_elegidos if c in COMPONENTES_KITS]
    if not comps_validos:
        return {} if isinstance(categorias, list) else ""
    
    # Normalizar categorias a lista
    if categorias is None:
        cats_a_generar = list(TEMPLATES_DESCRIPCION_KIT.keys())
        return_dict = True
    elif isinstance(categorias, str):
        cats_a_generar = [categorias]
        return_dict = False
    else:
        cats_a_generar = categorias
        return_dict = True
    ...
    for cat in cats_a_generar:
        if cat not in TEMPLATES_DESCRIPCION_KIT:
            continue
        
        template = TEMPLATES_DESCRIPCION_KIT[cat]
        
        # Lista de nombres cortos
        nombres_cortos = [COMPONENTES_KITS[c]["descripcion_corta"] for c in comps_validos]
        componentes_lista = " y ".join(nombres_cortos)
        
        # "con resto" solo si hay múltiples componentes
        if len(comps_validos) > 1:
            otros = [c for c in comps_validos if c != comps_validos[0]]
            resto_corto = " y ".join([COMPONENTES_KITS[c]["descripcion_corta"] for c in otros])
            con_resto = f" con kit de auxilio vehicular compuesto por {resto_corto}"
            complementos = f" El kit se complementa con {resto_corto}."
        else:
            con_resto = ""
            complementos = ""
        ...
        con_comps = ""
        if len(comps_validos) > 1:
            otros_nombres = [COMPONENTES_KITS[c]["nombre"].upper() for c in comps_validos[1:]]
            if otros_nombres:
                con_comps = " CON " + " Y ".join(otros_nombres)
        
        titulo = template["titulo_format"].format(con_componentes=con_comps).strip()
        
        # Intro
        intro = template["intro"].format(
            componentes_lista=componentes_lista,
            con_resto=con_resto,
        ...
        # Incluye
        incluye = ""
        if template.get("include_list"):
            items = []
            for comp_key in comps_validos:
                comp_desc_corta = COMPONENTES_KITS[comp_key]["descripcion_corta"]
                items.append(f"1 unidad de {comp_desc_corta}.")
            if items:
                incluye = "EL KIT PUBLICADO INCLUYE\n\n" + "\n\n".join(items)
        
        # Fichas técnicas
        fichas = "\n\n".join([COMPONENTES_KITS[c]["descripcion_tecnica"] for c in comps_validos])
        
        # Usos (genérico para cualquier kit)
        usos_beneficios = """USOS RECOMENDADOS

Cambio de neumáticos y recambio de rueda de emergencia.
Auxilio vial y uso preventivo en el baúl.
Inflado de neumáticos y control de presión.
Mantenimiento básico del vehículo.
Viajes y uso cotidiano.


BENEFICIOS DEL KIT

La combinación de herramientas y elementos permite contar con una solución funcional para tareas de auxilio vial, mantenimiento vehicular y uso preventivo en el baúl.

El formato compacto de los componentes facilita su transporte y almacenamiento dentro del vehículo.

Ideal para llevar en emergencias, viajes, uso cotidiano y situaciones de auxilio vial.


APLICACIÓN UNIVERSAL Y MEDIDAS DE REFERENCIA

El kit es apto para usos generales en automóviles particulares, SUVs compactos y vehículos ligeros, sujeto a las especificaciones y puntos de apoyo indicados por el fabricante de cada vehículo.

Antes de comprar, verificá las especificaciones técnicas de cada componente y su compatibilidad con tu vehículo.

La aplicación puede abarcar vehículos de marcas como General Motors, Chevrolet, Volkswagen, PSA, Peugeot, Citroën, Toyota, Ford, Nissan, Fiat y Renault, entre otras, siempre que las características del vehículo sean compatibles con las especificaciones de las herramientas publicadas."""
        
        # Armar descripción
        partes = [titulo, "", intro]
        if incluye:
            partes.extend(["", incluye])
        partes.extend(["", fichas, "", usos_beneficios, "", CIERRE_ESTANDAR])
        
        resultado[cat] = "\n\n".join(partes)
    
    # Devolver según tipo de entrada
    if return_dict:
        return resultado
    else:
        return resultado.get(cats_a_generar[0], "")
...
# Valores fijos FORZADOS por categoría: se escriben tal cual en el Publicar,
# aunque el AGENTE no traiga esa columna o la traiga como "solicitar".
# Clave: categoría normalizada -> { header normalizado: valor a escribir }.
# El header debe coincidir (normalizado) con el de la hoja del Publicar de Meli.
VALORES_FIJOS_FORZADOS_AP = {
    "crique hidraulico": {
        "formato del crique": "Carrito",
    },
    "compresor de aire": {
        "codigo universal de producto": "El producto es un kit o un pack",
    },
}
...
def parsear_agente_publicador_ap(archivo_bytes):
    ...
    for idx, (fila_titulo, nombre_cat, sku_bloque) in enumerate(titulos):
        ...
        # --- Valores fijos forzados (ej. Crique: "Formato del crique" = Carrito) ---
        fijos_forzados = VALORES_FIJOS_FORZADOS_AP.get(normalizar_texto(nombre_limpio), {})
        for hn_target, valor_fijo in fijos_forzados.items():
            # Si estaba como "solicitar", lo saco (pasa a ser fijo)
            for c in list(campos_solicitar):
                if c["header_norm"] == hn_target:
                    campos_solicitar.remove(c)
            # Reemplazo el fijo si existía, o lo agrego
            existente = next(
                (c for c in campos_fijos if c["header_norm"] == hn_target), None
            )
            if existente is not None:
                existente["valor"] = valor_fijo
            else:
                campos_fijos.append({"header_norm": hn_target, "valor": valor_fijo})
...
def _precio_a_entero(valor):
    """
    Devuelve el precio como número ENTERO (sin decimales ni separadores de
    miles), para que se escriba como número en la celda del Publicar.
    Acepta "67285", "67.285,00", "$ 67.285", 67285.0, etc.
    Si no se puede interpretar como número, devuelve el valor original tal
    cual (para que el usuario note el problema en vez de perder el dato).
    """
    numero = convertir_precio_a_numero(valor)
    if numero is None:
        ...
        for campo in bloque["campos_solicitar"]:
            if campo["es_titulo"]:
                ...
                if col and col not in cols_no_tocar:
                    ...
                    # Precio: siempre número entero (sin decimales ni puntos)
                    if "precio" in campo["header_norm"]:
                        valor = _precio_a_entero(valor)
                    ws.cell(fila, col).value = valor

    return len(titulos)


def _es_campo_compartido_ap(campo):
    """SKU y Precio son del kit: se cargan una vez y valen para todas las categorías."""
    hn = campo["header_norm"]
    return hn == "sku" or "precio" in hn
...
def mostrar_herramienta_agente_publicador():
    ...
    # ============================================================
    # GENERADOR DE DESCRIPCIONES DE KITS (nuevo)
    # ============================================================
    with st.expander("📝 Generador de Descripciones de Kits (nuevo)", expanded=False):
        st.caption("Genera descripciones automáticas para kits según sus componentes y categoría.")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.subheader("Componentes del Kit")
            comp_crique = st.checkbox("Crique Hidráulico Tipo Carrito 2T", key="gen_crique")
            comp_compresor = st.checkbox("Compresor Portátil 12V", key="gen_compresor")
        
        with col2:
            st.subheader("Categoría a publicar")
            categoria_desc = st.radio(
                "Enfoque de la descripción",
                ["kit_seguridad", "compresor", "crique"],
                format_func=lambda x: {
                    "kit_seguridad": "Kit Seguridad",
                    "compresor": "Compresor (enfoque en compresor)",
                    "crique": "Crique (enfoque en crique)",
                }[x],
                key="gen_categoria"
            )
        ...
        componentes_sel = []
        if comp_crique:
            componentes_sel.append("crique_carrito_2t")
        if comp_compresor:
            componentes_sel.append("compresor_12v")
        
        if st.button("Generar Descripción", type="primary", key="gen_desc_btn"):
            if not componentes_sel:
                st.error("Elegí al menos un componente.")
            else:
                desc_generada = generar_descripcion_kit(componentes_sel, categoria_desc)
                if desc_generada:
                    st.success("Descripción generada.")
                    with st.expander("📋 Ver y copiar descripción", expanded=True):
                        st.text_area(
                            "Descripción completa (copiar y pegar en el campo Descripción del Publicar)",
                            value=desc_generada,
                            height=400,
                            disabled=False,
                            key="gen_desc_output"
                    ...
                    st.error("No se pudo generar la descripción.")

    # ============================================================
    # RESTO DEL AGENTE PUBLICADOR (original)
    # ============================================================
    ...
    # ============================================================
    # MODO KIT: un mismo kit publicado en varias categorías, en una
    # sola pasada. SKU y Precio se cargan una vez (son del kit).
    # Título, Descripción, Fotos y campos propios van por categoría.
    # ============================================================

    # Pre-selección: solo categorías cuya hoja NO se repite (evita que
    # dos bloques escriban sobre la misma hoja y se pisen).
    conteo_hojas = {}
    for b in bloques_disponibles:
        conteo_hojas[b["hoja_publicar"]] = conteo_hojas.get(b["hoja_publicar"], 0) + 1
    default_sel = [
        b["etiqueta"] for b in bloques_disponibles
        if conteo_hojas[b["hoja_publicar"]] == 1
    ]

    st.subheader("1) ¿A qué categorías va este kit?")
    ...
    bloques_sel = [b for b in bloques_disponibles if b["etiqueta"] in seleccionadas]

    # --- 2) Datos compartidos del kit ---
    st.subheader("2) Datos del kit (se aplican a todas las categorías)")
    col_a, col_b = st.columns(2)
    sku_compartido = col_a.text_input("SKU del kit", key="ap_sku_kit")
    precio_compartido = col_b.text_input("Precio [$]", key="ap_precio_kit")

    # --- 3) Por categoría: título, descripción, fotos y campos propios ---
    st.subheader("3) Completá cada categoría")
    st.caption("SKU y Precio ya salen de arriba. Acá va lo que adaptás en cada una.")
    ...
    for bloque in bloques_sel:
        ...
        with st.expander(
            f"{etiqueta}  →  hoja '{bloque['hoja_publicar']}'",
            expanded=len(bloques_sel) == 1,
        ):
            valores = {}
            ...
            for campo in campos_no_titulo:
                ...
                # SKU / Precio: vienen de la sección compartida, no se re-piden
                if _es_campo_compartido_ap(campo):
                    if campo["header_norm"] == "sku":
                        valores[label] = sku_compartido
                    else:
                        valores[label] = precio_compartido
                    continue

                k = f"ap_{etiqueta}_{label}"
                if tipo == "select_sino":
                    valores[label] = st.selectbox(label, ["Sí", "No"], key=k)
                elif "descripci" in label.lower():
                    valores[label] = st.text_area(
                        label, height=150, key=k,
                        placeholder="Descripción para esta categoría"
                    )
                elif "foto" in label.lower():
                    valores[label] = st.text_area(
                        label, height=80, key=k,
                        placeholder="Links de fotos separados por coma"
                    )
                else:
                    valores[label] = st.text_input(label, key=k)

            titulos_texto = st.text_area(
                "Títulos (uno por línea)",
                height=140,
                key=f"ap_tit_{etiqueta}",
                placeholder="Un título por línea, adaptado a esta categoría",
            )
            titulos = [t.strip() for t in titulos_texto.strip().split("\n") if t.strip()]
            if titulos:
                st.caption(f"{len(titulos)} títulos.")

            datos_por_categoria[etiqueta] = {"titulos": titulos, "valores": valores}

    # --- Preview de lo que se va a escribir ---
    st.divider()
    resumen_prev = " · ".join(
        f"{e}: {len(d['titulos'])} pub." for e, d in datos_por_categoria.items()
    )
    st.caption(f"Se van a escribir → {resumen_prev}")

    # --- Generar todo el Publicar de una ---
    if st.button("Generar Publicar completo", type="primary", key="ap_generar_multi"):

        # Guardia: dos categorías a la misma hoja se pisarían
        hojas_sel = {}
        for bloque in bloques_sel:
            hojas_sel.setdefault(bloque["hoja_publicar"], []).append(bloque["etiqueta"])
        colisiones = {h: e for h, e in hojas_sel.items() if len(e) > 1}
        if colisiones:
            detalle = "; ".join(
                f"hoja '{h}' ← {', '.join(es)}" for h, es in colisiones.items()
            )
            st.error(
                "Elegiste varias categorías que van a la misma hoja del Publicar "
                f"({detalle}). Dejá solo una por hoja y volvé a generar."
            )
            return

        cats_con_titulos = [e for e, d in datos_por_categoria.items() if d["titulos"]]
        if not cats_con_titulos:
            st.error("Ninguna categoría tiene títulos cargados. Cargá al menos uno.")
            return

        if not sku_compartido.strip() or not precio_compartido.strip():
            st.warning("Ojo: falta SKU o Precio del kit. Se escribe igual, pero revisá.")
        ...
        try:
            ...
            for bloque in bloques_sel:
                etiqueta = bloque["etiqueta"]
                d = datos_por_categoria[etiqueta]
                if not d["titulos"]:
                    resumen.append((etiqueta, 0))
                    continue
                n = escribir_publicaciones_ap(
                    wb, bloque["hoja_publicar"], bloque, d["titulos"], d["valores"]
                )
        ...
        cats_escritas = len([r for r in resumen if r[1] > 0])
        st.success(f"{total} publicaciones escritas en {cats_escritas} categorías.")
        for e, n in resumen:
            marca = "·" if n > 0 else "—"
            st.caption(f"{marca} {e}: {n} publicaciones")
...
# ============================================================
# HERRAMIENTA: CARGA DE DATOS (Artículos / Descripciones / Precios)
# ============================================================
...
_ARTICULOS_PATH = os.path.join(_APP_DIR, "articulos.json")

# Categorías que usan campo "Modelo" en el Publicar
CATEGORIAS_MODELO = ["kit_seguridad", "compresor", "crique"]
CATEGORIAS_MODELO_LABEL = {
    "kit_seguridad": "Kit Seguridad",
    "compresor": "Compresor",
    "crique": "Crique",
}
...
def _ruta_articulos():
    """Busca articulos.json en el repo (raíz o data/)."""
    for d in _ARTICULOS_DIRS:
        p = os.path.join(d, "articulos.json")
        if os.path.exists(p):
            return p
    return None
...
def cargar_articulos():
    """Carga artículos: sesión -> repo -> vacío."""
    if "articulos_data" in st.session_state:
        return st.session_state["articulos_data"]
    ruta = _ruta_articulos()
    if ruta:
        try:
            with open(ruta, "r", encoding="utf-8") as f:
                data = json.load(f)
            st.session_state["articulos_data"] = data
            return data
        except (OSError, json.JSONDecodeError):
            pass
    st.session_state["articulos_data"] = {}
    return {}


def guardar_articulos(data):
    """Guarda en sesión y en disco (persiste solo si el disco no es efímero)."""
    st.session_state["articulos_data"] = data
    try:
        with open(_ARTICULOS_PATH, "w", encoding="utf-8") as f:
...
def _slug_articulo(nombre):
    """Genera una key interna a partir del nombre."""
    s = normalizar_texto(nombre)
    s = re.sub(r'[^a-z0-9]+', '_', s).strip('_')
    return s or "articulo"


def mostrar_herramienta_carga_datos():
    st.title("Carga de Datos")
    st.caption("Administrá tus artículos, descripciones y lista de precios.")

    articulos = cargar_articulos()

    # Aviso de persistencia
    if _ruta_articulos() is None:
        st.info(
            "No hay articulos.json en el repo todavía. Podés cargar artículos acá "
            "y bajar el JSON para committearlo. En Streamlit Cloud, la persistencia "
            "real es tener articulos.json en el repo."
        )

    submodulo = st.radio(
        "Submódulo",
        ["Artículos", "Descripciones", "Lista de Precios"],
        horizontal=True,
        key="cd_submodulo",
    )
    ...
    # SUBMÓDULO: ARTÍCULOS
    # ========================================================
    if submodulo == "Artículos":
        st.subheader("Artículos")

        modo = st.selectbox(
            "Acción",
            ["Agregar artículo", "Modificar artículo"],
            key="cd_art_modo",
        )

        # Datos precargados si es modificación
        art_actual = {}
        key_actual = None
        if modo == "Modificar artículo":
            if not articulos:
                st.warning("No hay artículos cargados todavía.")
                return
            opciones = {f"{v.get('sku','?')} — {v.get('nombre','?')}": k for k, v in articulos.items()}
            elegido = st.selectbox("Elegí el artículo a modificar", list(opciones.keys()), key="cd_art_sel")
            key_actual = opciones[elegido]
            art_actual = articulos[key_actual]

        # Formulario
        col1, col2 = st.columns(2)
        nombre = col1.text_input(
            "Nombre del artículo",
            value=art_actual.get("nombre", ""),
            placeholder="Ej: Compresor de Aire Portátil 12V",
            key="cd_art_nombre",
        )
        sku = col2.text_input(
            "SKU",
            value=art_actual.get("sku", ""),
            placeholder="Ej: COM001",
            key="cd_art_sku",
        )

        precio = st.number_input(
            "Precio base",
            value=float(art_actual.get("precio", 0.0)),
            min_value=0.0,
            step=100.0,
            format="%.2f",
            key="cd_art_precio",
        ...
        desc_corta = st.text_input(
            "Descripción corta (para frases tipo 'incluye X y Z')",
            value=art_actual.get("descripcion_corta", ""),
            placeholder="Ej: compresor de aire portátil 12v con manómetro",
            key="cd_art_desc_corta",
        )

        st.markdown("**Modelo por categoría** (lo que se completa en el campo Modelo del Publicar)")
        modelo_cat = {}
        cols_m = st.columns(len(CATEGORIAS_MODELO))
        modelos_actuales = art_actual.get("modelo_por_categoria", {})
        for i, cat in enumerate(CATEGORIAS_MODELO):
            modelo_cat[cat] = cols_m[i].text_input(
                CATEGORIAS_MODELO_LABEL[cat],
                value=modelos_actuales.get(cat, ""),
                placeholder="Ej: Portátil 12V",
                key=f"cd_art_modelo_{cat}",
            )

        fotos = st.text_area(
            "Fotos (links separados por coma) — opcional",
            value=art_actual.get("fotos", ""),
            placeholder="https://foto1.jpg, https://foto2.jpg",
            height=70,
            key="cd_art_fotos",
        )

        desc_tecnica = st.text_area(
            "Descripción técnica (specs y medidas)",
            value=art_actual.get("descripcion_tecnica", ""),
            placeholder="NOMBRE DEL PRODUCTO\n\n[descripción]\n\nMEDIDAS APROXIMADAS\n- ...",
            height=300,
            key="cd_art_desc_tecnica",
        )

        if st.button("Guardar artículo", type="primary", key="cd_art_guardar"):
            if not nombre.strip() or not sku.strip():
                ...
                nuevo = {
                    ...
                    "modelo_por_categoria": modelo_cat,
                    "fotos": fotos.strip(),
                    "descripcion_tecnica": desc_tecnica,
                }
                key = key_actual if key_actual else _slug_articulo(nombre)
                # Evitar pisar otra key existente al agregar
                if modo == "Agregar artículo" and key in articulos:
                    key = f"{key}_{sku.strip().lower()}"
                articulos[key] = nuevo
                persistido = guardar_articulos(articulos)
                st.success(f"Artículo '{nombre}' guardado ({len(articulos)} en total).")
                if not persistido:
                    st.warning("No se pudo escribir en disco (entorno efímero). Bajá el JSON para committearlo.")

        # Descargar JSON para committear
        if articulos:
            st.divider()
            json_str = json.dumps(articulos, ensure_ascii=False, indent=2)
            st.download_button(
                "Descargar articulos.json (para subir al repo)",
    ...
    # ========================================================
    # SUBMÓDULO: DESCRIPCIONES
    # ========================================================
    elif submodulo == "Descripciones":
        st.subheader("Descripciones — bloques fijos y estructura")
        st.caption(
            "Estos bloques se usan al generar descripciones automáticas de kits. "
            "Podés editar el texto y elegir en qué orden aparecen."
        )

        bloques_default = cargar_bloques_descripcion()

        st.markdown("**Bloques de texto fijos**")
        nuevos_bloques = {}
        for bkey, binfo in bloques_default.items():
            with st.expander(f"{binfo['titulo']}", expanded=False):
                activo = st.checkbox(
                    "Incluir este bloque",
                    value=binfo.get("activo", True),
                    key=f"cd_desc_activo_{bkey}",
                )
                texto = st.text_area(
                    "Contenido",
                    value=binfo.get("texto", ""),
                    height=180,
                    key=f"cd_desc_texto_{bkey}",
                )
                orden = st.number_input(
                    "Orden (menor = más arriba)",
                    value=int(binfo.get("orden", 0)),
                    step=1,
                    key=f"cd_desc_orden_{bkey}",
                )
                nuevos_bloques[bkey] = {
                    "titulo": binfo["titulo"],
                    "texto": texto,
        ...
        if st.button("Guardar descripciones", type="primary", key="cd_desc_guardar"):
            persistido = guardar_bloques_descripcion(nuevos_bloques)
            st.success("Bloques de descripción guardados.")
            if not persistido:
                st.warning("No se pudo escribir en disco. Bajá el JSON para committearlo.")

        bloques_actuales = st.session_state.get("bloques_desc_data", bloques_default)
        json_str = json.dumps(bloques_actuales, ensure_ascii=False, indent=2)
        st.download_button(
            "Descargar descripciones.json (para subir al repo)",
            data=json_str.encode("utf-8"),
            file_name="descripciones.json",
            mime="application/json",
            key="cd_desc_download",
        )

    # ========================================================
    # SUBMÓDULO: LISTA DE PRECIOS
    # ========================================================
    elif submodulo == "Lista de Precios":
        st.subheader("Lista de Precios")
        st.caption(
            "Generá una plantilla Excel con tus SKUs, completá los precios y volvé "
            "a subirla para actualizar todo de una."
        )

        # --- Markup / suma fija ---
        st.markdown("**Ajuste de precio (opcional)**")
        col_a, col_b = st.columns(2)
        tipo_ajuste = col_a.selectbox(
            "Tipo de ajuste sobre el precio cargado",
            ["Sin ajuste", "Monto fijo ($)", "Porcentaje (%)"],
            key="cd_precio_tipo_ajuste",
        )
        valor_ajuste = col_b.number_input(
            "Valor del ajuste",
            value=0.0,
            step=1.0,
            key="cd_precio_valor_ajuste",
        )

        st.divider()

        # --- Generar plantilla Excel ---
        st.markdown("**1) Generar plantilla Excel**")
        if st.button("Generar Excel de precios", key="cd_precio_generar"):
            excel_bytes = generar_excel_precios(articulos)
            st.download_button(
                "Descargar plantilla de precios",
                data=excel_bytes,
                file_name="lista_precios.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="cd_precio_download_template",
            )
            st.success("Plantilla generada. Completá las columnas de precio y volvé a subirla abajo.")

        st.divider()

        # --- Cargar Excel para actualizar ---
        st.markdown("**2) Subir Excel completado para actualizar precios**")
        archivo_precios = st.file_uploader(
            "Excel de precios (SKU, ML Clásica, ML Premium)",
            type=["xlsx"],
            key="cd_precio_upload",
        )

        if archivo_precios is not None:
            resultado = actualizar_precios_desde_excel(
                articulos, archivo_precios.getvalue(), tipo_ajuste, valor_ajuste
            )
            if resultado["ok"]:
                st.success(
                    f"{resultado['actualizados']} precios actualizados"
                    + (f", {resultado['no_encontrados']} SKU sin match." if resultado['no_encontrados'] else ".")
                )
                if resultado["detalle"]:
                    with st.expander("Ver detalle de cambios"):
                        for linea in resultado["detalle"]:
                            st.caption(linea)
                if st.button("Confirmar y guardar precios", type="primary", key="cd_precio_confirmar"):
                    guardar_articulos(articulos)
                    st.success("Precios guardados en los artículos.")
            else:
                st.error(resultado["error"])


# ---- Helpers de Descripciones ----

_BLOQUES_DESC_PATH = os.path.join(_APP_DIR, "descripciones.json")

BLOQUES_DESC_DEFAULT = {
    "usos": {
        "titulo": "Usos recomendados",
        "orden": 1,
        "activo": True,
        "texto": "USOS RECOMENDADOS\n\nCambio de neumáticos y recambio de rueda de emergencia.\nAuxilio vial y uso preventivo en el baúl.\nInflado de neumáticos y control de presión.\nMantenimiento básico del vehículo.\nViajes y uso cotidiano.",
    },
    "beneficios": {
        "titulo": "Beneficios del kit",
        "orden": 2,
        "activo": True,
        "texto": "BENEFICIOS DEL KIT\n\nLa combinación de herramientas y elementos permite contar con una solución funcional para tareas de auxilio vial, mantenimiento vehicular y uso preventivo en el baúl.\n\nEl formato compacto de los componentes facilita su transporte y almacenamiento dentro del vehículo.\n\nIdeal para llevar en emergencias, viajes, uso cotidiano y situaciones de auxilio vial.",
    },
    "aplicacion": {
        "titulo": "Aplicación universal",
        "orden": 3,
        "activo": True,
        "texto": "APLICACIÓN UNIVERSAL Y MEDIDAS DE REFERENCIA\n\nEl kit es apto para usos generales en automóviles particulares, SUVs compactos y vehículos ligeros, sujeto a las especificaciones y puntos de apoyo indicados por el fabricante de cada vehículo.\n\nAntes de comprar, verificá las especificaciones técnicas de cada componente y su compatibilidad con tu vehículo.\n\nLa aplicación puede abarcar vehículos de marcas como General Motors, Chevrolet, Volkswagen, PSA, Peugeot, Citroën, Toyota, Ford, Nissan, Fiat y Renault, entre otras, siempre que las características del vehículo sean compatibles con las especificaciones de las herramientas publicadas.",
    },
    "facturacion": {
        "titulo": "Facturación",
        "orden": 4,
        "activo": True,
        "texto": "FACTURACIÓN AUTOMATIZADA\n\nEmitimos Factura A y B. El proceso está automatizado y tomará directamente los datos fiscales cargados en tu cuenta al momento de ejecutar la compra.",
    },
    "logistica": {
        "titulo": "Logística y despacho",
        "orden": 5,
        "activo": True,
        "texto": "LOGÍSTICA Y DESPACHO INMEDIATO\n\nEnvíos Flex: compras previas a las 13:00 hs se entregan en el día, CABA y GBA, entre las 16:00 hs y 22:00 hs.\n\nMercado Envíos: despachamos a todo el país en horario comercial con embalaje de alta densidad para proteger los productos durante el tránsito.",
    },
    "garantia": {
        "titulo": "Garantía",
        "orden": 6,
...
def cargar_bloques_descripcion():
    if "bloques_desc_data" in st.session_state:
        return st.session_state["bloques_desc_data"]
    for d in _ARTICULOS_DIRS:
        p = os.path.join(d, "descripciones.json")
        if os.path.exists(p):
            try:
                with open(p, "r", encoding="utf-8") as f:
                    data = json.load(f)
                st.session_state["bloques_desc_data"] = data
                return data
            except (OSError, json.JSONDecodeError):
                pass
    st.session_state["bloques_desc_data"] = BLOQUES_DESC_DEFAULT
    return BLOQUES_DESC_DEFAULT


def guardar_bloques_descripcion(data):
    st.session_state["bloques_desc_data"] = data
    try:
        with open(_BLOQUES_DESC_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        return True
    except OSError:
        return False
...
def generar_excel_precios(articulos):
    """Genera un Excel con columnas SKU, Nombre, ML Clásica, ML Premium."""
    from openpyxl import Workbook
    wb = Workbook()
    ws = wb.active
    ws.title = "Precios"
    headers = ["SKU", "Nombre", "ML Clásica", "ML Premium"]
    for c, h in enumerate(headers, 1):
        ws.cell(1, c).value = h
    fila = 2
    for art in articulos.values():
        ws.cell(fila, 1).value = art.get("sku", "")
        ws.cell(fila, 2).value = art.get("nombre", "")
        ws.cell(fila, 3).value = art.get("precio", "")
...
def _aplicar_ajuste(precio, tipo, valor):
    try:
        precio = float(precio)
    except (TypeError, ValueError):
        return precio
    if tipo == "Monto fijo ($)":
        return round(precio + valor, 2)
    if tipo == "Porcentaje (%)":
        return round(precio * (1 + valor / 100.0), 2)
    return round(precio, 2)


def actualizar_precios_desde_excel(articulos, excel_bytes, tipo_ajuste, valor_ajuste):
    """Lee el Excel y actualiza precios en articulos por SKU. Usa ML Clásica como precio base."""
    try:
    ...
    for c in range(1, ws.max_column + 1):
        v = ws.cell(1, c).value
        if v:
            headers[normalizar_texto(v)] = c
    col_sku = headers.get("sku")
    col_clasica = headers.get("ml clasica") or headers.get("precio")
    if not col_sku or not col_clasica:
        return {"ok": False, "error": "El Excel debe tener columnas 'SKU' y 'ML Clásica'."}

    # Índice sku -> key de artículo
    sku_a_key = {normalizar_texto(v.get("sku", "")): k for k, v in articulos.items()}

    actualizados = 0
    no_encontrados = 0
    detalle = []
    for fila in range(2, ws.max_row + 1):
        sku_val = ws.cell(fila, col_sku).value
        precio_val = ws.cell(fila, col_clasica).value
        if sku_val is None or precio_val is None:
            continue
        key = sku_a_key.get(normalizar_texto(str(sku_val)))
        if not key:
            no_encontrados += 1
            continue
        precio_final = _aplicar_ajuste(precio_val, tipo_ajuste, valor_ajuste)
        anterior = articulos[key].get("precio", 0)
        articulos[key]["precio"] = precio_final
        detalle.append(f"{sku_val}: {anterior} → {precio_final}")
        actualizados += 1
...
elif herramienta == "Carga de Datos":
    mostrar_herramienta_carga_datos()
