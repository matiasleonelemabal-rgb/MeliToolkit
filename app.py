...
Su capacidad de carga es de 2 Toneladas, equivalente a 2000 kg. Cuenta con mecanismo hidráulico, válvula de seguridad y ruedas traseras giratorias que facilitan el posicionamiento del crique debajo del vehículo.

Su formato tipo carrito permite desplazar y acomodar la herramienta durante la maniobra de elevación.

MEDIDAS APROXIMADAS DEL CRIQUE CARRITO
...
MEDIDAS APROXIMADAS DE LA PALANCA

Largo de palanca: 38 cm aprox.
Diámetro de palanca: 1,8 cm aprox.
Diámetro del agarre de palanca: 2,3 cm aprox.""",
    },
    "compresor_12v": {
        "nombre": "Compresor de Aire Portátil 12V",
        "descripcion_corta": "compresor de aire portátil 12V",
        "descripcion_tecnica": """COMPRESOR DE AIRE PORTÁTIL 12V

El compresor de aire portátil 12V permite realizar tareas de inflado de forma práctica utilizando la alimentación eléctrica del vehículo.

Cuenta con cuerpo fabricado en metal, aleación de aluminio y componentes plásticos. Posee manómetro incorporado en la parte superior del equipo para controlar la presión durante el inflado.

Incluye botón de encendido y apagado ON/OFF, manija superior para facilitar el agarre y transporte, manguera de aire y cable de alimentación con conexión para ficha de encendedor 12V.

El kit incorpora 3 picos adaptadores de inflado que amplían sus posibilidades de uso en diferentes elementos inflables.

CARACTERÍSTICAS DEL COMPRESOR PORTÁTIL 12V

Voltaje: DC 12V.
Amperaje máximo: 10 Amp.
Presión máxima: 140 PSI.
Desplazamiento de aire: 35 L/min.
Diámetro del cilindro: 30 mm.
Manómetro de presión incorporado.
Botón de encendido y apagado ON/OFF.
Manija superior de transporte.
Conexión a ficha de encendedor 12V.
Incluye 3 picos adaptadores de inflado.

MEDIDAS APROXIMADAS DEL COMPRESOR PORTÁTIL 12V

Largo: 16 cm aprox.
Alto: 14 cm aprox.
Ancho: 8 cm aprox.

MEDIDAS APROXIMADAS DEL CABLE DE ALIMENTACIÓN 12V

Largo: 260 cm aprox.

MEDIDAS APROXIMADAS DE LA MANGUERA DE AIRE
...
Este conjunto reúne herramientas y elementos de apoyo para responder ante diferentes situaciones de uso en ruta, ciudad, cochera o viaje, combinando elevación del vehículo, inflado portátil y, según la variante publicada, herramientas de rueda y elementos de visibilidad o señalización.
...
Pensado para quienes buscan centralizar en un solo kit distintos elementos de auxilio vial y uso preventivo dentro del vehículo.""",
        "include_list": True,
        "order_componentes": ["crique_carrito_2t", "compresor_12v"],
    },
    "compresor": {
...
Ideal para llevar en el baúl como equipo preventivo para viajes, uso cotidiano y asistencia mecánica básica.""",
        "include_list": True,
        "order_componentes": ["compresor_12v", "crique_carrito_2t"],
    },
    "crique": {
        "titulo_format": "CRIQUE HIDRÁULICO TIPO CARRITO 2 TONELADAS {con_componentes}",
        "intro": """Crique hidráulico tipo carrito 2 Toneladas {con_resto}. El crique es la herramienta principal de esta publicación y permite elevar el vehículo de forma estable y controlada durante cambio de rueda, mantenimiento básico y tareas de auxilio vial.
...
def generar_descripcion_kit(componentes_elegidos, categoria):
    """
    Genera descripción completa del kit basada en componentes y categoría.
    componentes_elegidos: lista de keys ["crique_carrito_2t", "compresor_12v", ...]
    categoria: "kit_seguridad", "compresor", "crique"
    """
    if not componentes_elegidos or categoria not in TEMPLATES_DESCRIPCION_KIT:
        return ""

    template = TEMPLATES_DESCRIPCION_KIT[categoria]
    comps_validos = [c for c in componentes_elegidos if c in COMPONENTES_KITS]
    if not comps_validos:
        return ""

    # Ordenar según template
    orden = template.get("order_componentes", comps_validos)
    comps_ordenados = [c for c in orden if c in comps_validos]

    # Construir lista de nombres
    nombres_cortos = [COMPONENTES_KITS[c]["descripcion_corta"] for c in comps_ordenados]
    componentes_lista = " y ".join(nombres_cortos)

    # Construir "con resto"
    if len(comps_ordenados) > 1:
        resto_corto = " y ".join([COMPONENTES_KITS[c]["descripcion_corta"] for c in comps_ordenados[1:]])
        con_resto = f"con kit de auxilio vehicular compuesto por {resto_corto}"
        complementos = f"El kit se complementa con {resto_corto} para tareas relacionadas con cambio de rueda y mantenimiento básico."
    else:
        con_resto = ""
        complementos = ""

    # Título
    con_comps = f"CON {' Y '.join([COMPONENTES_KITS[c]['nombre'].upper() for c in comps_ordenados[1:]])}" if len(comps_ordenados) > 1 else ""
    ...
    if template.get("include_list"):
        items = ["1 unidad de crique hidráulico tipo carrito 2 Toneladas."] if "crique_carrito_2t" in comps_ordenados else []
        items += ["1 unidad de compresor de aire portátil 12V."] if "compresor_12v" in comps_ordenados else []
        items += ["1 kit de 3 picos adaptadores de inflado."] if "compresor_12v" in comps_ordenados else []
        if items:
            incluye = "EL KIT PUBLICADO INCLUYE\n\n" + "\n\n".join(items)

    # Fichas técnicas de componentes en orden
    fichas = "\n\n".join([COMPONENTES_KITS[c]["descripcion_tecnica"] for c in comps_ordenados])
...
El crique hidráulico permite elevar el vehículo y facilita las tareas relacionadas con el cambio de rueda.
El compresor portátil permite realizar tareas de inflado utilizando la alimentación 12V del vehículo.
El formato de los componentes facilita su transporte y almacenamiento dentro del baúl.
La combinación de los elementos permite contar con una solución funcional para tareas básicas de auxilio vial y mantenimiento vehicular.
...
Antes de comprar, verificá que la capacidad de carga y el rango de elevación del crique sean adecuados para tu vehículo.

Para utilizar el compresor, verificá las especificaciones de presión recomendadas por el fabricante del neumático o elemento a inflar.
