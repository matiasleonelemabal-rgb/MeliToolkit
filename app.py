...
# Sufijo que se agrega al SKU base según la categoría de publicación.
# El SKU se carga una vez (ej. KIT2TC-5) y cada categoría escribe el suyo:
# Kit -> KIT2TC-5 (sin sufijo), Crique -> KIT2TC-5CR, Compresor -> KIT2TC-5COM, etc.
# Claves: nombre limpio de categoría normalizado (case/acentos-insensitive).
SUFIJOS_SKU_AP = {
    "kit de seguridad para auto": "",
    "crique hidraulico": "CR",
    "compresor de aire": "COM",
    "llave cruz / llave de rueda": "L",
    "chaleco reflectivo": "CHA",
    "baliza": "BAL",
    "cono de seguridad vial": "CON",
}


def sku_para_categoria_ap(sku_base, nombre_limpio):
    """Devuelve el SKU con el sufijo de la categoría. Sin match conocido, devuelve el base."""
    base = (sku_base or "").strip()
    if not base:
        return base
    sufijo = SUFIJOS_SKU_AP.get(normalizar_texto(nombre_limpio), "")
...
def mostrar_herramienta_agente_publicador():
    ...
    for bloque in bloques_sel:
        ...
        with st.expander(
            ...
            if sku_compartido.strip():
                st.caption(
                    f"SKU para esta categoría: "
                    f"**{sku_para_categoria_ap(sku_compartido, bloque['nombre_limpio'])}**"
                )
            valores = {}
            ...
            for campo in campos_no_titulo:
                ...
                # El SKU lleva el sufijo de la categoría (Kit sin sufijo,
                # Crique CR, Compresor COM, Llave L, Chaleco CHA, Baliza BAL, Cono CON).
                if _es_campo_compartido_ap(campo):
                    if campo["header_norm"] == "sku":
                        valores[label] = sku_para_categoria_ap(
                            sku_compartido, bloque["nombre_limpio"]
                        )
                    else:
