...
CARPETA_AGENTE_AP = "agente_publicador_data"


def _guardar_agente_ap(archivo_bytes):
    os.makedirs(CARPETA_AGENTE_AP, exist_ok=True)
    ruta = os.path.join(CARPETA_AGENTE_AP, "AGENTE_PUBLICADOR.xlsx")
    with open(ruta, "wb") as f:
        f.write(archivo_bytes)


def _ruta_agente_ap():
    return os.path.join(CARPETA_AGENTE_AP, "AGENTE_PUBLICADOR.xlsx")


def _agente_existe():
    return os.path.exists(_ruta_agente_ap())
...
def parsear_agente_publicador_ap(archivo_bytes):
    ...
    for h in wb.sheetnames:
        if "completar" in normalizar_texto(h):
            hoja_datos = h
            break

    if hoja_datos is None:
        raise ValueError(
            "El AGENTE no tiene hoja 'celdas a completar'."
        )
    ...
    for idx, (fila_titulo, nombre_cat) in enumerate(titulos):
...
def encontrar_hoja_publicar(wb, nombre_limpio):
    """
    Busca la hoja del Publicar que corresponde a esta categoría.
    Primero intenta el mapeo conocido, luego búsqueda fuzzy.
    """
    hoja_esperada = MAPEO_CATEGORIAS_AP.get(nombre_limpio)

    if hoja_esperada and hoja_esperada in wb.sheetnames:
        return hoja_esperada

    # Fuzzy: buscar por nombre normalizado
    nombre_norm = normalizar_texto(nombre_limpio)
    for hoja in wb.sheetnames:
        if hoja in ("Ayuda", "extra info"):
            continue
        if nombre_norm in normalizar_texto(hoja) or normalizar_texto(hoja) in nombre_norm:
            return hoja
...
def mostrar_herramienta_agente_publicador():
    ...
    # --- Subir AGENTE (una vez, se guarda) ---
    with st.expander("Configurar AGENTE_PUBLICADOR (solo la primera vez)"):
        st.caption(
            "Subí el AGENTE_PUBLICADOR.xlsx con la hoja 'celdas a completar'. "
            "Se guarda y no hace falta volver a subirlo."
        )
        archivo_agente = st.file_uploader(
            "AGENTE_PUBLICADOR.xlsx",
            type=["xlsx"],
    ...
    # Parsear AGENTE
    with open(_ruta_agente_ap(), "rb") as f:
        agente_bytes = f.read()
    ...
    try:
        wb_pub = load_workbook(BytesIO(archivo_publicar.getvalue()))
    except Exception as e:
        st.error("No se pudo abrir el Publicar.")
        st.exception(e)
        return

    hojas_publicar = [h for h in wb_pub.sheetnames if h not in ("Ayuda", "extra info")]

    # Matchear bloques del AGENTE con hojas del Publicar
    bloques_disponibles = []
    for b in bloques:
        hoja = encontrar_hoja_publicar(wb_pub, b["nombre_limpio"])
        if hoja:
