...
def parsear_agente_publicador_ap(archivo_bytes):
    ...
    for idx, (fila_titulo, nombre_cat, sku_bloque) in enumerate(titulos):
        ...
        for hn_target, tipo in override.items():
            # Resolver el header REAL presente en este bloque. Por defecto se
            # exige igualdad exacta normalizada (para no tocar Título / Modelo
            # detallado). Excepción acotada: para "fotos" se permite match por
            # substring, porque la columna puede llamarse "Fotos",
            # "Fotos del producto", etc., y ninguna otra columna que contenga
            # "foto" debería quedar fija.
            hn_real = hn_target
            if hn_target not in mapa_header_texto and hn_target == "fotos":
                cand = next((hn for hn in mapa_header_texto if "foto" in hn), None)
                if cand is not None:
                    hn_real = cand

            # Si ya es un campo solicitar, solo fijo su tipo (ej. select Sí/No)
            ya = next((c for c in campos_solicitar if c["header_norm"] == hn_real), None)
            if ya is not None:
                ya["tipo"] = tipo
                continue
            # Si estaba como fijo, lo saco de fijos y lo paso a solicitar
            fijo = next((c for c in campos_fijos if c["header_norm"] == hn_real), None)
            if fijo is not None:
                campos_fijos.remove(fijo)
            # Construyo el campo solicitar (aunque el dato estuviera vacío)
            if hn_real in mapa_header_texto:
                label = mapa_header_texto[hn_real]
                if len(label) > 40:
                    label = label[:40]
                campos_solicitar.append({
                    "header_norm": hn_real,
